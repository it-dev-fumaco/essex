...
trait AttendanceTrait
{
    ...
    /**
     * Preload shift details for a date range and shift_group to avoid N+1 in loops.
     * Returns a map: date_string => shift_details (object with time_in, grace_period_in_mins, etc.)
     */
    public function getShiftScheduleMap($from_date, $to_date, $shift_group){
        ...
        $dayNames = array_unique(array_map(fn ($d) => Carbon::parse($d)->format('l'), $dates));
        $shiftsByDay = DB::table('shifts')
            ->where('shift_group_id', $shift_group)
            ->whereIn('day_of_week', $dayNames)
            ->get()
            ->keyBy('day_of_week');
        $specialByDate = DB::table('shift_schedule')
            ->whereBetween('sched_date', [reset($dates), end($dates)])
            ->get()
            ->keyBy(fn ($r) => Carbon::parse($r->sched_date)->format('Y-m-d'));
        $map = [];
        foreach ($dates as $dateStr) {
            $special = $specialByDate->get($dateStr);
            if ($special) {
                $map[$dateStr] = $special;
            } else {
                $dayName = Carbon::parse($dateStr)->format('l');
                $map[$dateStr] = $shiftsByDay->get($dayName);
            }
        }
        return $map;
    }
    ...
    public function getBiometricLogs($user_id, $from_date, $to_date){
        ...
        $shiftScheduleMap = $this->getShiftScheduleMap($from_date, $to_date, $shift_group);
        ...
        foreach ($biometric as $row) {
            foreach ($row as $d) {
                $shift_details = $shiftScheduleMap[$d->bio_date] ?? $this->getShiftSchedule($d->bio_date, $shift_group);
                $late_in_mins = 0;
                if (!empty($d->locout) && $shift_details) {
                    $late_in_mins = $this->getLatesInMins($d->timein, $shift_details->time_in ?? null, $shift_details->grace_period_in_mins ?? 0, $d->bio_date, $user_id);
                }
                $data[] = [
                    'bio_date' => $d->bio_date,
                    'time_in' => $d->timein,
    ...
    public function attendanceLogs($employee, $date_from, $date_to){
        ...
        $sorted = $employee_logs;
        asort($sorted);
        $employee_logs = array_reverse(array_values($sorted));
