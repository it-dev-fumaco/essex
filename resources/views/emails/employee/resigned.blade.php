...
@section('title', 'Resigned employee — ' . config('app.name'))
    ...
    @php
        ...
        try {
            if (!empty($data['resignation_date'])) {
                $preheaderDate = \Carbon\Carbon::parse($data['resignation_date'])->format('M d, Y');
            }
        } catch (\Throwable $e) {
        }
    @endphp
    Resigned employee: revoke access and recover assets{{ $preheaderDate !== '' ? ' by ' . $preheaderDate . '.' : '.' }}
@endsection
...
@section('email_heading')
    Resigned employee — action required
@endsection

@section('content')
    @php
        $resignationFormatted = '';
        try {
            if (!empty($data['resignation_date'])) {
                $resignationFormatted = \Carbon\Carbon::parse($data['resignation_date'])->format('F d, Y');
            }
        } catch (\Throwable $e) {
            $resignationFormatted = $data['resignation_date'] ?? '';
        }
    @endphp
    ...
    <p style="margin:0 0 20px;">
        An employee has resigned. Please begin the standard offboarding procedure and complete account deactivation and hardware recovery
        @if ($resignationFormatted !== '')
            by <strong style="color:#0f172a;">{{ $resignationFormatted }}</strong>
        @endif
        ...
        <tr>
            <td style="padding:14px 16px; background-color:#f1f5f9; border-radius:6px; border:1px solid #e2e8f0;">
                ...
                @if (!empty($data['employee_id']))
                    <p style="margin:0 0 6px;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Employee ID</span> <strong style="color:#0f172a;">{{ $data['employee_id'] }}</strong></p>
                @endif
                @if (!empty($data['biometric_id']))
                    <p style="margin:0 0 6px;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Access ID</span> <strong style="color:#0f172a;">{{ $data['biometric_id'] }}</strong></p>
                @endif
                ...
                <p style="margin:0 0 6px;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Position</span> <strong style="color:#0f172a;">{{ $data['job_title'] ?? ($data['designation'] ?? ($data['role'] ?? '')) }}</strong></p>
                ...
                @if (!empty($data['reporting_to']))
                    <p style="margin:0 0 6px;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Reporting manager</span> <strong style="color:#0f172a;">{{ $data['reporting_to'] }}</strong></p>
                @endif
                <p style="margin:0 0 6px;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Primary location</span> <strong style="color:#0f172a;">{{ $data['primary_location'] ?? ($data['location'] ?? '') }}</strong></p>
                @if ($resignationFormatted !== '')
                    <p style="margin:0;"><span style="display:inline-block; min-width:132px; color:#64748b; font-size:13px;">Resignation date</span> <strong style="color:#0f172a;">{{ $resignationFormatted }}</strong></p>
                @endif
            </td>
        </tr>
    </table>
    ...
    <p style="margin:0 0 8px;"><span style="color:#64748b; font-size:13px; display:inline-block; min-width:140px;">Email</span> Disable Microsoft 365 / Zimbra; set auto-reply if requested by manager.</p>
    <p style="margin:0 0 8px;"><span style="color:#64748b; font-size:13px; display:inline-block; min-width:140px;">ERP</span> Deactivate user; revoke module permissions.</p>
